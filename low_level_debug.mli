...
val start_canary_thread : max_wait:float -> check_interval:float -> unit
(** A canary thread starts two threads, one in caml and one in C. The caml
    ...
    the canary thread to support this, so it may stop your program again. If you
    do manage to continue, the canary thread should continue to work.
...
(** Pretty-print any type any type. Note that this will likely infinite-loop and
    overflow the stack for any value whose structure is circularly recursive. *)

val obj_to_string : 'a -> string
(** Obtain a string representation of any type. Note that this will likely
    infinite-loop and overflow the stack for any value whose structure is
    circularly recursive. *)
